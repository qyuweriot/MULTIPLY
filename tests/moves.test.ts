...
import { makeCard, makeState } from './helpers.ts'

/** current(=p0) の手札を差し替えた盤面を作る */
function withHand(state: GameState, ...ids: CardId[]): GameState {
  return { ...state, hands: [ids.map(makeCard), state.hands[1]] }
}
