 ...
 *
 * 引き分けはない（同点は先攻の勝ち）。ties は「得点が並んだ試合」の診断値で、
 * wins / losses のどちらかにも数えられている。
 */
export function winRate(
  difficulties: [Difficulty, Difficulty],
  games: number,
): { wins: number; losses: number; ties: number; rate: number } {
  let wins = 0
  let losses = 0
  let ties = 0
  ...
  for (let i = 0; i < games; i++) {
    ...
    const { winner, tied } = result(end)
    const target: PlayerId = swapped ? 1 : 0

    if (tied) ties++
    if (winner === target) wins++
    else losses++
  }

  return { wins, losses, ties, rate: wins / games }
}
